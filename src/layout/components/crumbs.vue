<template>
  <div class="crumbs">
    <template v-for="item in routeHistory" :key="item.path">
      <div
        :class="['crumbs_item', { crumbs_item_active: item.path === currentRoute.path }]"
        @click="onCrumbsClick(item.path)"
      >
        <span>{{ item.name }}</span>
        <el-icon @click.stop="onDeleteRouteHistory(item.path)" v-if="routeHistory.length > 1"
          ><Close
        /></el-icon>
      </div>
    </template>
